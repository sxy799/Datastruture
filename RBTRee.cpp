/*************************************************************************
	...
	> Created Time: 2018年03月23日 星期五 18时47分02秒
 ************************************************************************/
#include <stdio.h>
#include <stdlib.h>
...
RBTNode *delete_maintain(RBTNode *root) {
    ...
    if (root->lchild->color == 0) {//左兄弟节点是红色
        root = right_rotate(root);
        root->color = 1, root->rchild->color = 0;
        root->rchild = delete_maintain(root->rchild);
        return root;
    } else if (root->rchild->color == 0) {//右兄弟节点是红色
        root = left_rotate(root);
        root->color = 1, root->lchild->color = 0;
    ...
    if (root->lchild->color == 2) {
        if (root->rchild->rchild->color && root->rchild->lchild->color) {///兄弟节点是黑色的，而且兄弟节点的两个孩子也是黑色的
            root->rchild->color = 0;
            root->color += 1;
            root->lchild->color = 1;
        } else {
            root->lchild->color = 1;
            if (root->rchild->rchild->color) {//兄弟节点的左孩子是红色
                root->rchild = right_rotate(root->rchild);
                root->rchild->color = 1;
